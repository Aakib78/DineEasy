...
Same kind of real-client-only bug as the `seed.ts` fix above, just caught one step later (at `nest start --watch`'s `tsc` pass instead of `ts-node`'s) — `prisma:migrate:dev` and `seed` had already succeeded for real by the time this surfaced. This sandbox's stub client can't confirm the fix compiles clean either (same limitation); pending confirmation from a real `npm run dev:api` run.
