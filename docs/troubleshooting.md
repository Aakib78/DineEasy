...
**Confirmed live**: item 2 above (`POSTGRES_PASSWORD is required` from a bare `docker compose -f ... down` run by hand, despite `.env` having a perfectly good value) reproduced exactly as predicted during real end-to-end testing. Prefer `npm run docker:down` (and `docker:up`/`docker:prod`) over typing `docker compose -f ...` yourself for this exact reason — they already carry `--env-file .env`.
...
### Fixed in this repo: `npm run dev:all` fails with `Fatal error during bootstrap: Error: listen EADDRINUSE: address already in use 0.0.0.0:3000`

Caught on the very next real run after the `usb` fix above got `dev:all` further than before. Two distinct causes, found in order:

**1. `dev:all` itself used to call the wrong Docker script.** It originally ran the plain `docker:up` (no service names), which brings up the *whole* dockerized stack — including a containerized `api` bound to host port 3000 — at the same time `concurrently` starts the host's own `dev:api` (`nest start --watch`) on that same port. `docs/local-development.md`'s step 2 already documented, before `dev:all` existed, that day-to-day work should only bring up `postgres redis` and explicitly warned against the unscoped `docker:up` for this exact reason — a warning not cross-referenced when `dev:all` was first authored. Fixed: added `docker:up:infra` (`docker compose ... up -d postgres redis`, identical to step 2's own command) and repointed `dev:all` to call it instead. `docker:up` itself is untouched and still correct for its actual purpose (`docker:prod`/full dockerized-parity checks) — the bug was only in which script `dev:all` reached for.

**2. Fixing (1) alone isn't enough if a dockerized `api`/`web`/`pos` from an earlier plain `docker:up` (or `docker:prod`) is still running** — `dev:all`'s new `docker:up:infra` only ever touches `postgres`/`redis`, so it neither starts nor stops those three. Symptom: `dev:all` still fails with the same `EADDRINUSE`, even after the fix in (1), even after running `npm run stop:all` — because in the case actually observed, the leftover containers had survived *both* `stop:all` (its `docker:down` step apparently ran before Docker Desktop itself was restarted a few minutes later, which auto-resumed them) and looked, from `lsof`, like the port was held by Docker Desktop's own backend/proxy process (`com.docker...`) rather than anything obviously named after the project — which it's *not* safe to `kill` directly, since that's shared Docker Desktop infrastructure, not a per-container process, and killing it can destabilize Docker Desktop entirely rather than just freeing the port.

Diagnosis: `docker ps` — if it lists `dineeasy-api-1`/`dineeasy-web-1`/`dineeasy-pos-1` (not just `dineeasy-postgres-1`/`dineeasy-redis-1`), that's the leftover full stack, whatever started it. Fix: stop and remove just those three (leave postgres/redis, which `dev:all` still wants) —

```bash
docker stop dineeasy-api-1 dineeasy-pos-1 dineeasy-web-1
docker rm dineeasy-api-1 dineeasy-pos-1 dineeasy-web-1
```

— or, for a full clean slate, `npm run docker:down` (stops all five, `dev:all`'s `docker:up:infra` will recreate postgres/redis on the next run). Confirm the ports are actually clear afterward with `lsof -nP -iTCP:3000,3001,5173 -sTCP:LISTEN` before retrying `npm run dev:all`.
...
Root cause: `ItemCustomizeSheet`'s `handleAdd` called `crypto.randomUUID()` directly to generate each cart line's id. `crypto.randomUUID` only exists in a browser "secure context" — HTTPS, or the special-cased `localhost` — and this app is deliberately served over plain HTTP on the restaurant's LAN (docs/architecture.md §1, no TLS in v1). Loaded at `http://localhost:5173/...` on the same machine running the dev server, `localhost` itself makes it a secure context, so this never failed during development. Loaded at its real LAN address on a phone (`http://192.168.1.49:5173/...`, never `localhost`), it is **not** a secure context, `crypto.randomUUID` is simply `undefined`, and calling it threw a `TypeError` inside the click handler — silently, since nothing in the UI surfaces a caught-nowhere exception from an `onClick`. This is the exact same class of gap as `docs/customer-web.md`'s service-worker limitation (also secure-context-gated), except this one broke a core ordering flow rather than just an install prompt.

Fixed by routing cart-line id generation through `lib/id.ts`'s `generateId()` instead — tries `crypto.randomUUID()` first (works fine wherever it's available, e.g. `localhost` during development or a future TLS deployment), and falls back to building a v4 UUID from `crypto.getRandomValues` otherwise, which — unlike the convenience wrapper — *is* available in an insecure context. Not yet re-verified on a real device (needs a fresh `npm run dev:web` pull + phone retest); this sandbox can confirm the code builds/lints clean but has no way to reproduce a secure-vs-insecure-context difference itself, since everything here goes through `localhost`.

**Worth checking for next time**: any other browser API gated the same way (`crypto.subtle`, `navigator.clipboard`, geolocation, etc.) would fail identically — silently, and only on a real device over the LAN, never in local development against `localhost`. This is a real blind spot specific to this project's LAN-first-over-HTTP architecture, not something `npm run build`/`lint` can ever catch.
