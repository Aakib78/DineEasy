...
**Retry**: a `FAILED` job (one that's exhausted `updateJobStatus`'s automatic 3-attempt retry above) can now be retried by staff directly from either screen — `POST /printers/jobs/:jobId/retry` (`PrintersService.retryJob`) resets it to `{status: QUEUED, attempts: 0, lastError: null}`, a fresh start the agent's next poll picks up, and records a `printer_job.retried` audit entry. Only legal on a currently-`FAILED` job — the backend 400s otherwise (`QUEUED` is already going to be tried; `SENT`/`ACKED` already succeeded, so "retrying" either would just print the same ticket twice) — surfaced as an inline error on the row rather than a generic failure. `PrinterJobsScreen.tsx` shows a "Retry" button per `FAILED` row; `printer_jobs_screen.dart` mirrors it. Used to require a direct database edit to unstick a job; this closes that gap.

**Still not built**: no way to *cancel* a job (there's no `CANCELLED` value in the `PrinterJobStatus` enum — adding one is a schema migration, not attempted here), and no backend-level pagination past the fixed 50-row window or a dedicated queue-depth/summary aggregate endpoint — both screens compute everything client-side over what `GET /printers/:id/jobs` already returns.
