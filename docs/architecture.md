...
See root `README.md` for the directory tree. `packages/shared_types` (an npm workspace, `@dineeasy/shared-types`) holds hand-written TypeScript types mirroring `services/api`'s JSON response shapes, consumed by `apps/customer_web` (whose own `src/lib/api/types.ts` is now just a re-export of it — see `docs/customer-web.md`). "Hand-written" rather than generated from the OpenAPI spec `@nestjs/swagger` now produces (`docs/api.md`) because generating from it means bootstrapping the live API, which needs a working `@prisma/client` this sandbox can't produce (`docs/troubleshooting.md`) — switching to spec-generated types is the natural next step once that's no longer true, and nothing about how these are written fights that migration. The Flutter app has its own Dart models (`lib/features/*/data/*_models.dart`, hand-mirrored per screen) since Dart can't consume a TypeScript package regardless of how it's produced — see `docs/flutter-app.md`.
...
- Every permission-gated feature-area destination in the Flutter nav shell now has a real screen — POS, Tables management, Kitchen/KDS, Billing, Reports, and Staff (all above) are done; only Settings remains a placeholder (expected to stay minimal — the spec has no dedicated settings requirements beyond sign-out, already in the nav shell itself).
